...
for profile_id in range(start_id-200,end_id-200):
    ...
    command = f'instaloader --post-filter="date_utc >= {start_date} and date_utc <= {end_date}" {profile_name} --no-metadata-json --no-profile-pic --no-video-thumbnails --dirname-pattern={folder_name} '
    
    #command = "dir"
    os.system(f'cmd /c {command}')              #to close 
    #command = f'instaloader --post-filter="date_utc >= datetime(2017, 1, 1) and date_utc <= datetime(2018, 3, 30) " justmangobrown --no-metadata-json --no-profile-pic --dirname-pattern={folder_name}'
